...
### ⚠️ 말투 규칙 (최우선 필수 - 반드시 지켜야 합니다)

**모든 나레이션 문장은 반드시 존댓말(~습니다/~지요/~셨습니다)로 끝나야 합니다.**

화자는 시청자에게 친절하고 따뜻하게 이야기하는 사람입니다.
절대로 반말을 사용하지 마세요.
...
**존댓말 나레이션 예시:**
- ❌ "눈시울이 붉어졌다" → ✅ "눈시울이 붉어졌습니다"
- ❌ "가슴이 먹먹해졌다" → ✅ "가슴이 먹먹해졌지요"
- ❌ "한숨 소리가 퍼졌다" → ✅ "한숨 소리가 퍼졌습니다"
- ❌ "그런데 그날..." → ✅ "그런데요, 그날..."
- ❌ "운명은 다른 계획을 갖고 있었다" → ✅ "운명은 다른 계획을 갖고 있었습니다"
- ✅ "그렇습니다, 우리 마음은 이렇지요"
- ✅ "참 대단하셨습니다"
- ✅ "누구나 한 번쯤은 이런 순간이 있지 않을까요?"

**반말 완전 금지 어미:** "~했다", "~이었다", "~한다", "~된다", "~일까", "~았다", "~었다"

※ 대사(직접 인용문)만 상황에 맞게 반말 가능. 나레이션은 100% 존댓말.
...
{"- **나레이션은 반드시 존댓말(~습니다/~지요)로 작성하세요. 반말(~했다/~이었다) 절대 금지**" if style == "자유모드" else ""}
...
## 📌 클릭 최적화 핵심 전략 (3S Strategy - 모든 스타일 공통 필수)

제목과 썸네일은 **'입구(클릭률)'**를 넓히기 위해 대중의 **결핍을 건드려야** 합니다.
모든 스타일(불교/스토리텔링/자유모드/명상/뉴스/영어/일본어)에 반드시 아래 3가지 원칙을 적용하세요.

### 1) Simple (단순성)
- 어려운 전문 용어(불교 용어, 철학 용어, 한자어) 대신 **초등학생도 이해할 일상 언어** 사용
- 예: "무상(無常)" → "다 변하는 이유", "집착" → "놓지 못하는 마음", "업보" → "돌아오는 대가"
- 예: "번뇌" → "머릿속이 복잡할 때", "자아" → "진짜 나"

### 2) Sharp (날카로움)
- "좋은 말씀", "감동 이야기" 같은 뭉뚱그린 표현 금지
- **"지금 당장 내 문제"**처럼 느껴지게 구체적 상황으로 치환
- 예: ❌ "보시의 공덕" → ✅ "운이 들어오기 전 반드시 일어나는 변화"
- 예: ❌ "경전 공부의 태도" → ✅ "아무리 공부해도 인생이 안 바뀌는 사람 특징"

### 3) Self-reference (자기 참조)
- 시청자가 **"이거 내 얘기인데?"** 라고 느끼도록 지시어 사용
- 필수 지시어: **"당신", "나", "지금", "~하는 사람"**
- 예: "만만하게 보이기 싫다면 반드시 끊어야 할 관계"
- 예: "생각이 너무 많아 괴롭다면 딱 이것만 하세요"

---

## 📌 제목 생성 치트시트 (전문 주제 → 일상 고민 치환)

전문 주제를 **일상 고민(인간관계, 불안, 돈, 자존감)**으로 변환하세요:

| 원주제 | 클릭 유도 변환 | 핵심 키워드 |
|--------|---------------|------------|
| 자비/무소유/인간관계 | "만만하게 보이기 싫다면 끊어야 할 관계" | 손절, 거리두기, 만만한 사람 |
| 망상/번뇌 | "생각이 너무 많아 괴롭다면 딱 이것만 하세요" | 잡생각, 뇌 정지, 불면증 |
| 보시/공덕/운 | "운이 들어오기 전 반드시 일어나는 변화들" | 징조, 운명, 인생 역전 |
| 경전 공부/깨달음 | "아무리 공부해도 인생 안 바뀌는 사람 특징" | 헛수고, 가짜 공부 |
| 감동/교훈 스토리 | "듣고 나면 생각이 바뀌는 이야기" | 실화, 인생, 깨달음 |

### 권장 제목 구조 패턴
- **부정형/경고형**: "~하지 않으면 반드시 후회하는 것", "~인 사람 특징"
- **해답 제시형**: "~하는 이유", "~하는 법", "~할 때 딱 이것만 하세요"
- **결핍 자극형**: "~하고 싶다면", "~이 안 풀린다면"
- **금기어**: 제목 앞에 "부처님 가르침", "좋은 말씀", "오늘의 명언" 같은 도입부 절대 금지 → **본질적인 문장으로 바로 시작**

---

## 📌 썸네일 카피 추가 원칙 (8자 이내 강렬 카피)

기본 상단 텍스트(2~6자) 외에, 아래 패턴도 적극 활용:
- **질문 던지기**: "왜 나만?", "이게 맞나?"
- **충격 주기**: "이걸 몰랐다니", "그만 참으세요"
- **변화 약속**: "인생이 편해짐", "이제 그만"
