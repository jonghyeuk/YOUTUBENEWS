...
ELEVENLABS_STYLE_VOICES = {
    ...
    "자유모드": {
        "voice_id": "zgDzx5jLLCqEp6Fl7Kl7",  # 감성 스토리텔러 음성
        "stability": 0.25,  # Creative - 극적이고 다이나믹한 톤
        "similarity_boost": 0.75,
        "speed": 0.9,  # 약간 느리게 (감정 살리기)
    },
}
...
EMOTION_TAGS = {
    ...
    "자유모드": {
        "intro": "[curious, engaging]",      # 흥미로운 시작 (훅)
        "body": "[warm, storytelling]",      # 이야기꾼처럼 전달
        "body_tense": "[tense, dramatic]",   # 갈등/긴장 고조
        "climax": "[emotional, deep]",       # 절정 (감정 폭발)
        "ending": "[thoughtful, wise]",      # 교훈/깨달음 전달
    },
}
...
FFMPEG_FILTERS = {
    ...
    "subtitle_style": {
        ...
        "shadow": 2,  # 그림자 (더 진하게)
        "margin_v": 150,  # 하단 마진 (잘림 방지를 위해 더 위로)
...
YOUTUBE_TITLE_TEMPLATES = {
    ...
    "자유모드": [
        "이 이야기, 끝까지 들어보세요 ({duration}분)",
        "누구에게도 말 못한 이야기 {duration}분",
        "인생이 달라지는 {duration}분의 깨달음",
        "듣고 나면 생각이 바뀝니다 ({duration}분)",
        "이런 사연, 처음 들어보실 거예요 {duration}분",
        "마음이 울리는 {duration}분 이야기",
        "잠들기 전, 이 이야기 하나만 ({duration}분)",
        "생각할수록 깊어지는 이야기 {duration}분",
    ],
}
...
YOUTUBE_DEFAULT_TAGS = {
    ...
    "자유모드": "감동,실화,인생이야기,교훈,깨달음,스토리텔링,이야기,사연,인생,동기부여,힐링,위로,공감,눈물,감성,마음,자기계발,명언,잠잘때듣는,ASMR",
}
...
#수면명상 #가이드명상 #깊은잠 #이완명상
""",
    "자유모드": """이야기 하나가 마음을 움직입니다.
오늘 밤, 잠들기 전 들으면 좋은 이야기입니다.
...
📌 채널 안내
삶을 돌아보게 하는 이야기, 마음에 남는 깨달음을 전합니다.
이야기꾼처럼 들려드리는 감동 콘텐츠입니다.

👍 공감되셨다면 구독/좋아요 부탁드립니다.
💬 여러분의 이야기도 댓글로 들려주세요.
