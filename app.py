...
## ⚠️ 언어 규칙 (최우선 필수)
- **출력은 100% 한국어**로만 작성하세요.
- **영어 단어를 절대로 사용하지 마세요.** 사용자 입력이 영어라도, 영어를 그대로 쓰거나 괄호 안에 병기하지 마세요.
- ❌ 잘못된 예: "말(words)", "자아(ego)", "침묵(silence)", "갈등(conflict)"
- ✅ 올바른 예: "말", "자아", "침묵", "갈등"
- 괄호 안에 원어 영어를 병기하는 행위를 **완전히 금지**합니다.
- 외래어가 꼭 필요하면 한국어 표기(한글)만 사용하세요. 예: "에고" (O) / "에고(ego)" (X)
